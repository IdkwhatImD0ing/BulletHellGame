	...
	public void fire(Point2D point) {
		if (visible) {
			board.getEnemyProjectiles().add(new Projectile(x + getWidth() / 2, y + getHeight() / 2, point.getX(),
					point.getY(), "src/Enemies/Zombie/Bullet3.png", 700, 2, 15 + 2 * board.getRound()));
			board.getEnemyProjectiles().add(new Projectile(x + getWidth() / 2, y + getHeight() / 2,
					getTargetAngle() - ANGLE, "src/Enemies/Zombie/Bullet3.png", 700, 2, 15 + 2 * board.getRound()));
			board.getEnemyProjectiles().add(new Projectile(x + getWidth() / 2, y + getHeight() / 2,
					getTargetAngle() + ANGLE, "src/Enemies/Zombie/Bullet3.png", 700, 2, 15 + 2 * board.getRound()));
			board.getEnemyProjectiles().add(new Projectile(x + getWidth() / 2, y + getHeight() / 2,
					getTargetAngle() + 2 * ANGLE, "src/Enemies/Zombie/Bullet3.png", 700, 2, 15 + 2 * board.getRound()));
			board.getEnemyProjectiles().add(new Projectile(x + getWidth() / 2, y + getHeight() / 2,
					getTargetAngle() - 2 * ANGLE, "src/Enemies/Zombie/Bullet3.png", 700, 2, 15 + 2 * board.getRound()));
		}
	}
